<div class="container main gap-above-large" id="about">
    <div class="row">
        <div class="col-md-12">
            <h1>Advisory Committee</h1>
        </div>
    </div>

<div class="row">
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/msa.jpg" alt="Prof. M.S. Ananth">
      <div class="caption">
        <h4>Prof. M.S. Ananth</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/Prof_Nagarajan.jpg" alt="Prof. R. Nagarajan">
      <div class="caption">
        <h4>Prof. R. Nagarajan</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/Mahesh_Panchagnula.jpg" alt="Prof. Mahesh Panchagnula">
      <div class="caption">
        <h4>Prof. Mahesh Panchagnula</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/Kolar_Photo.jpg" alt="Prof. Ajit Kolar">
      <div class="caption">
        <h4>Prof. Ajit Kolar</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/Koshy_Varghese.jpg" alt="Prof. Koshy Varghese">
      <div class="caption">
        <h4>Prof. Koshy Varghese</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/psmug.jpg" alt="Prof. P. Sriram">
      <div class="caption">
        <h4>Prof. P. Sriram</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/CSSwamy.jpg" alt="Prof. C.S. Swamy">
      <div class="caption">
        <h4>Prof. C.S. Swamy</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/santhakumar.jpg" alt="Prof. Santhakumar">
      <div class="caption">
        <h4>Prof. Santhakumar</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/Mr_S_Srinivasan.jpg" alt="Mr. S. Srinivasan">
      <div class="caption">
        <h4>Mr. S. Srinivasan</h4>
      </div>
    </div>
  </div>
  <div class="col-sm-6 col-md-3">
    <div class="thumbnail">
      <img src="<?=PUBLIC_URL?>images/stock/adcommittee/Kumaran.jpg" alt="Mr. Kumaran Sathasivam">
      <div class="caption">
        <h4>Mr. Kumaran Sathasivam</h4>
      </div>
    </div>
  </div>
</div>
</div>      



